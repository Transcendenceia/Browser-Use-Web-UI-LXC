...
# ─── Preguntas interactivas (modo no-quiet) ─────────────────────────────────────
if (( QUIET == 0 )); then
  ask CTID            "$CTID"            "CTID"
...
# ─── Plantilla Debian 12 ───────────────────────────────────────────────────────
LATEST=$(pveam available | awk '/debian-12-standard_.*amd64/ {print $2}' \
         | sort -Vr | head -n1)
...
if ! pveam list "$TEMPLATE_STORAGE" | grep -q "$LATEST"; then
  info "Descargando plantilla $LATEST a $TEMPLATE_STORAGE…"
...
# ─── Instalar dependencias + Google Chrome ─────────────────────────────────────
...
# ─── Playwright ────────────────────────────────────────────────────────────────
info "Preparando directorio /ms-playwright…"
ct_exec "mkdir -p /ms-playwright && chown $USERNAME:$USERNAME /ms-playwright"

# ─── Clonar browser-use/web-ui + venv ──────────────────────────────────────────
...
# ─── Ajustar directorios por defecto ───────────────────────────────────────────
info "Ajustando directorios por defecto…"
...
ct_exec "
  sudo -u $USERNAME -H \
    git clone https://github.com/novnc/noVNC.git /home/$USERNAME/web-ui/noVNC
"

# ─── Resolución por defecto ────────────────────────────────────────────────────
info "Parcheando resolución por defecto…"
ct_exec "
  sudo -u $USERNAME sed -i 's/value=1280/value=1920/' \
    /home/$USERNAME/web-ui/src/webui/components/browser_settings_tab.py
"
ct_exec "
  sudo -u $USERNAME sed -i 's/value=1100/value=1080/' \
    /home/$USERNAME/web-ui/src/webui/components/browser_settings_tab.py
"
...
info "Generando .env…"
ct_exec "
  cat > /home/$USERNAME/web-ui/.env <<EOF
OPENAI_API_KEY=$API_OPENAI
GOOGLE_API_KEY=$API_GOOGLE
ANTHROPIC_API_KEY=$API_ANTHROPIC
EOF
"
ct_exec "chown $USERNAME:$USERNAME /home/$USERNAME/web-ui/.env"

# ─── Supervisor ────────────────────────────────────────────────────────────────
info "Creando servicio Supervisor…"
ct_exec "
  cat > /etc/supervisor/conf.d/web-ui.conf <<'EOF'
[supervisord]
nodaemon=true
...
[program:webui]
priority=20
environment=DISPLAY=:0,PLAYWRIGHT_BROWSERS_PATH=/ms-playwright
directory=/home/$USERNAME/web-ui
user=$USERNAME
command=/home/$USERNAME/web-ui/venv/bin/python -m webui
autostart=true
autorestart=true
stdout_logfile=/var/log/webui.log
stderr_logfile=/var/log/webui.err
EOF
"

ct_exec "systemctl enable supervisor --now"

# ─── Mensaje final ─────────────────────────────────────────────────────────────
info "Instalación terminada ✅"
info "Abre tu navegador en http://<IP_del_CT>:7860 para usar Browser-Use Web-UI."
